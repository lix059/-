...
function findClosest(words, word1, word2) {
  ...
  words.forEach((w, idx) => {
    if (word1 === w) {
      wordIndexArr1.push(idx);
    } else if (word2 === w) {
      wordIndexArr2.push(idx);
    }
  });
  // TODO判断两个数组都存在数据
  let min = words.length;
  let distance;
  console.log(wordIndexArr1, wordIndexArr2);
  for (let i = 0, j = 0; i < wordIndexArr1.length && j < wordIndexArr2.length; ) {
    if (wordIndexArr1[i] < wordIndexArr2[j]) {
      distance = wordIndexArr2[j] - wordIndexArr1[i];
      if (distance < min) min = distance;
      i++;
    } else {
      distance = wordIndexArr1[i] - wordIndexArr2[j];
      if (distance < min) min = distance;
      j++;
    }
  }
